...
export async function updateProfile(
  ...
  if (cpf) {
    if (!validateCPF(cpf)) {
      return { error: 'CPF inválido. Verifique e tente novamente.', field: 'cpf' }
    }
    const existing = await prisma.profile.findFirst({
      where: { cpf, NOT: { id: user.id } },
      select: { id: true },
    })
    if (existing) return { error: 'Este CPF já está cadastrado por outro usuário.', field: 'cpf' }
  }

  // Atualiza senha se preenchida
  if (newPassword) {
    if (newPassword.length < 6) {
      return { error: 'A nova senha deve ter pelo menos 6 caracteres.', field: 'new_password' }
    }
